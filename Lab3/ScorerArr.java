...
public class ScorerArr {
	...
	public static void readScores (Scanner scores) {
		while (!gameIsOver ( )) {
			...
			if (batter == 1){
				visitorScores[inning - 1] = scores.nextInt();// use an array instead of just adding on to the value of visitorscores
				batter++;
			}
			else if(batter == 2){
				homeScores [inning - 1] = scores.nextInt();// no more adding onto the value
				batter--;
				inning++;
			}
		}
